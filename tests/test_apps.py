...
def test_discover_django_apps_finds_all_features_and_portals(settings):
    base_dir = settings.BASE_DIR
    discovered = discover_django_apps(base_dir)
    for app in EXPECTED_FEATURES + EXPECTED_PORTALS:
...
@pytest.mark.parametrize("app_name", EXPECTED_FEATURES + EXPECTED_PORTALS)
