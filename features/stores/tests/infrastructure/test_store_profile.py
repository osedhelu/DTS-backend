...
def test_update_store_profile_location(api_client):
    ...
    with override_settings(
        DATABASES=POSTGIS_DATABASE,
        INSTALLED_APPS=build_installed_apps(BACKEND_DIR),
        MEDIA_ROOT=BACKEND_DIR / "test_media_store_profile_location",
    ):
        merchant = CustomUser.objects.create_user(
            username="merchant_profile_location",
            email="profile_location@test.com",
            password="securepass123",
            role=UserRole.MERCHANT,
        )
        store = StoreModel(
            owner=merchant,
            name="Tienda Ubicación",
            status=StoreStatus.OPEN,
            vertical=StoreVertical.FOOD,
        )
        store.set_location(GeoLocation(latitude=4.7110, longitude=-74.0721))
        ...
        assert patch_response.status_code == status.HTTP_200_OK
        assert patch_response.data["latitude"] == pytest.approx(4.65)
        assert patch_response.data["longitude"] == pytest.approx(-74.05)

        store.refresh_from_db()
        assert store.latitude == pytest.approx(4.65)
        assert store.longitude == pytest.approx(-74.05)
